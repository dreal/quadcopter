...
%         % Address matrix SOS case: WITH decision variables
...
%     else
%         % Address matrix SOS case: NO decision variables
%         
...
%         [nmon,nvar] = size(Z);
%         
%         % Form coefficients for SOS equality constraint
%         coeffnts = sparse(nmon*dimp,dimp);
%         for i = 1:dimp
%             for j = i:dimp
...
%                     s_ijk = cij(nzidx(k));
%                     mon_k = Z(nzidx(k),:);
%                     ind_k = find( sum(Z==repmat(mon_k,[nmon,1]),2) == nvar );
%                     coeffnts((ind_k-1)*dimp+i,j) = s_ijk;
%                     coeffnts((ind_k-1)*dimp+j,i) = s_ijk;
%                 end
%             end
